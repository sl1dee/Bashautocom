...
;(function (global, factory) {
	...
	function mapToStyles(_ref2) {
		...
		const { popper } = _ref2;
		let { popperRect } = _ref2
		let { placement } = _ref2
		let { offsets } = _ref2
		let { position } = _ref2
		let { gpuAcceleration } = _ref2
		let { adaptive } = _ref2
		let { roundOffsets } = _ref2
	...
	function computeStyles(_ref4) {
		...
		let gpuAcceleration = _options$gpuAccelerat === void 0 ? true : _options$gpuAccelerat
		let _options$adaptive = options.adaptive
		let adaptive = _options$adaptive === void 0 ? true : _options$adaptive
		let _options$roundOffsets = options.roundOffsets
	...
	function effect(_ref) {
		...
		let scroll = _options$scroll === void 0 ? true : _options$scroll
		let _options$resize = options.resize
	...
	function isScrollParent(element) {
		...
		let {overflow} = _getComputedStyle
		let {overflowX} = _getComputedStyle
  ...
  */
	...
	function detectOverflow(state, options) {
		...
		let _options$placement = _options.placement
		let placement = _options$placement === void 0 ? state.placement : _options$placement
		let _options$boundary = _options.boundary
		let boundary = _options$boundary === void 0 ? clippingParents : _options$boundary
		let _options$rootBoundary = _options.rootBoundary
		let rootBoundary = _options$rootBoundary === void 0 ? viewport : _options$rootBoundary
		let _options$elementConte = _options.elementContext
		let elementContext = _options$elementConte === void 0 ? popper : _options$elementConte
		let _options$altBoundary = _options.altBoundary
		let altBoundary = _options$altBoundary === void 0 ? false : _options$altBoundary
		let _options$padding = _options.padding
	...
	function computeAutoPlacement(state, options) {
		...
		const _options = options
		let {placement} = _options
		let {boundary} = _options
		let {rootBoundary} = _options
		let {padding} = _options
		let {flipVariations} = _options
		let _options$allowedAutoP = _options.allowedAutoPlacements
	...
	function flip(_ref) {
		...
		let checkMainAxis = _options$mainAxis === void 0 ? true : _options$mainAxis
		let _options$altAxis = options.altAxis
		let checkAltAxis = _options$altAxis === void 0 ? true : _options$altAxis
		let specifiedFallbackPlacements = options.fallbackPlacements
		let {padding} = options
		let {boundary} = options
		let {rootBoundary} = options
		let {altBoundary} = options
		let _options$flipVariatio = options.flipVariations
		let flipVariations = _options$flipVariatio === void 0 ? true : _options$flipVariatio
	...
	function distanceAndSkiddingToXY(placement, rects, offset) {
		...
		const _ref = typeof offset === 'function' ? offset({ ...rects, placement: placement }) : offset
		let skidding = _ref[0]
		let distance = _ref[1]
	...
	function offset(_ref2) {
		...
		let {x} = _data$state$placement
		let { y } = _data$state$placement
	...
	function preventOverflow(_ref) {
		...
		let checkMainAxis = _options$mainAxis === void 0 ? true : _options$mainAxis
		let _options$altAxis = options.altAxis
		let checkAltAxis = _options$altAxis === void 0 ? false : _options$altAxis
		let {boundary} = options
		let {rootBoundary} = options
		let {altBoundary} = options
		let {padding} = options
		let _options$tether = options.tether
		let tether = _options$tether === void 0 ? true : _options$tether
		let _options$tetherOffset = options.tetherOffset
	...
	function popperGenerator(generatorOptions) {
		...
		let _generatorOptions$def = _generatorOptions.defaultModifiers
		let defaultModifiers = _generatorOptions$def === void 0 ? [] : _generatorOptions$def
		let _generatorOptions$def2 = _generatorOptions.defaultOptions
		...
		return function createPopper(reference, popper, options) {
			...
			var instance = {
				...
				forceUpdate: function forceUpdate() {
					...
					let {reference} = _state$elements
					let { popper } = _state$elements // Don't proceed if `reference` or `popper` are not valid elements
					// anymore
					...
					for (let index = 0; index < state.orderedModifiers.length; index++) {
						...
						let {fn} = _state$orderedModifie
						let _state$orderedModifie2 = _state$orderedModifie.options
						let _options = _state$orderedModifie2 === void 0 ? {} : _state$orderedModifie2
			...
			function runModifierEffects() {
				state.orderedModifiers.forEach(function (_ref3) {
					const { name } = _ref3;
					let _ref3$options = _ref3.options
					let options = _ref3$options === void 0 ? {} : _ref3$options
					const { effect } = _ref3
...
// # sourceMappingURL=bootstrap.bundle.js.yandex-map
